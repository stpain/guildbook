...
function GuildbookMixin:OnLoad()
    ...
    self.help.scrollChild.text:SetText([[
FAQ:

Where is my gear?
You might need to change your gear for the addon to scan it.
]])
    ...
    self.settings.scrollChild.resetCharacter.label:SetText(L["SETTINGS_RESET_CHARACTER_LABEL"])
    self.settings.scrollChild.resetGuild.label:SetText(L["SETTINGS_RESET_GUILD_LABEL"])
...
function GuildbookMixin:OnDatabaseInitialised()
    ...
    if showMinimapButton == false then
        self.MinimapIcon:Hide("Guildbook")
    else
        self.MinimapIcon:Show("Guildbook")
    end
...
function GuildbookMixin:OnGuildChanged(guild)
    ...
    self.guildName.label:SetText(self.selectedGuild:GetName())
    self.guildName:Show()
...
function GuildbookMixin:RosterListviewItem_OnMouseDown(character)
    ...
    self.guild.character.scrollChild.profileInfo.prof1:SetText(Tradeskills:GetLocaleNameFromID(character:GetTradeskill(1)) or "-")
    self.guild.character.scrollChild.profileInfo.prof2:SetText(Tradeskills:GetLocaleNameFromID(character:GetTradeskill(2)) or "-")
    self.guild.character.scrollChild.profileInfo.cooking:SetText(character:GetCookingLevel() or "-")
    self.guild.character.scrollChild.profileInfo.fishing:SetText(character:GetFishingLevel() or "-")
    self.guild.character.scrollChild.profileInfo.firstAid:SetText(character:GetFirstAidLevel() or "-")
