...
function Character:CreateFromData(guid, data)
    ...
    if type(data) == "table" then
        ...
        --need to find a few tradeskill bits
        ...
        --if the character had old data with english spec name convert to an ID
        --otherwise just default as 1 and 2
        local mainSpec, offSpec = 1, 2;
        local specializations = classData[data.Class].specializations;
        for k, spec in ipairs(specializations) do
            if data.MainSpec == spec then
                mainSpec = k;
            end
            if data.OffSpec == spec then
                offSpec = k;
            end
        end
...
function Character:GetData()
    ...
    data[self.data.profession1] = self.data.profession1Recipes
    data[self.data.profession2] = self.data.profession2Recipes
    return data;
end
